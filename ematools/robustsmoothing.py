...
def smoothn(y,nS0=10,axis=None,smoothOrder=2.0,sd=None,verbose=False,\
  ...
  Wtot = W;
  #print("W=", W)
  #print("Wtot=", Wtot, type(Wtot), Wtot.shape)
...
def gcv(p,Lambda,aow,DCTy,IsFinite,Wtot,y,nof,noe,smoothOrder):
    ...
    if np.any(aow>0.9): # aow = 1 means that all of the data are equally weighted
        ...
        #print("Isfinite=", IsFinite, type(IsFinite), IsFinite.shape)
        # take account of the weights to calculate RSS:
        yhat = dctND(Gamma*DCTy,f=idct);
        #WtiF = sqrt(Wtot[IsFinite])
        #yiF = y[IsFinite]
        #yhatiF = np.expand_dims(yhat[IsFinite], axis=1)
        #print("y=", y.shape)
        #print("yhat=", yhat.shape)
        #print("yiF=", yiF.shape, type(yiF))
        #print("yhatiF=", yhatiF.shape, type(yhatiF))
        #ys = yiF - yhatiF
        RSS = norm(sqrt(Wtot[IsFinite])*(y[IsFinite]-yhat[IsFinite]))**2;
        #RSS = norm(Wtot*(yiF-yhatiF))**2
    #---
    TrH = sum(Gamma);
